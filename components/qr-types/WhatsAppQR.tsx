...
import { WhatsAppQRInput, createWhatsAppQR } from '../../services/qrGenerators/whatsAppQR';
...
interface WhatsAppQRProps {
  initialData?: Partial<WhatsAppQRCode>;
  onSave: (qrCode: WhatsAppQRCode) => void;
  onCancel: () => void;
}
...
export const WhatsAppQR: React.FC<WhatsAppQRProps> = ({
  ...
  useEffect(() => {
    if (phoneNumber && countryCode && !label) {
      setLabel(`WhatsApp: ${formatPhone(countryCode, phoneNumber)}`);
    }
  ...
  const handleSave = () => {
    ...
    const whatsappQRInput: WhatsAppQRInput = {
      countryCode,
    ...
    const whatsappQRCode = initialData
      ? {
          ...initialData,
          ...whatsappQRInput,
      ...
      : createWhatsAppQR(whatsappQRInput);

    onSave(whatsappQRCode);
  };
